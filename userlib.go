...
Public Methods:
   DebugMsg(format, args ...interface)

Public Variables:
	SymbolicDebug: Set to true to enable Symbolic Debugger
	SymbolicVerbose: Set to true for very verbose symbols logging
	SymbolicMaxLength: An integer representing when to truncate a string (default: 5)
*/
...
func hmacEval(key []byte, msg []byte) ([]byte, error) {
	...
	record(res, `{"HMAC": {"key": %s, "msg": %s}}`, resolve(key), resolve(msg))
	record(res[:16], `{"HMAC[:16]": {"key": %s, "msg": %s}}`, resolve(key), resolve(msg))
...
func hashKDF(key []byte, msg []byte) ([]byte, error) {
	...
	record(res[:16], `{"HashKDF[:16]": {"key": %s, "msg": %s}}`, resolve(key), resolve(msg))
